...
def vol_cal(ret, cha_name, ret_freq_use: list):
    ...
    if 'Daily' not in ret_freq_use:
        raise ValueError("Calculating total volatility requires using 'Daily' returns.")
    ...
    vol_df = daily_ret_df.groupby(level=0).apply(calculate_volatility)
...
def cha_main(ret, cha_name, ret_freq_use: list):
    ...
    df_merge = merge_tables(ret, df_cha, cha_name)
    return df_merge
